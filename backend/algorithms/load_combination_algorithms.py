...
def compute_height_zone_width(building: Building, zone_number: int):
    if zone_number == 0:
        return building.get_zone(zone_number)[0].elevation
    else:
        prev_elevation = building.get_zone(zone_number - 1)[0].elevation
        curr_elevation = building.get_zone(zone_number)[0].elevation
...
def compute_height_zone_variables(building: Building, zone_number: int):
    return {'xn': compute_height_zone_width(building, zone_number),
            'hx': building.get_zone(zone_number)[0].elevation,
            'ce': building.get_zone(zone_number)[0].wind_load.factor.ce,
            'ax': building.get_zone(zone_number)[0].seismic_load.ax}
...
def uls_wall_1_4D(building: Building):
    return {'uls 1.4D': 1.4 * building.wp}
...
def uls_wall_1_25D_1_4Wy(building: Building, snow_load: SnowLoad, zone_num: int):
    return {'uls 1.25D': building.wp * 1.25,
            'uls 1.4Wy (centre)': building.get_zone(zone_num)[0].wind_load.get_zone('wall_centre').pressure.pos_uls,
            'uls 1.4Wy (edge)': building.get_zone(zone_num)[0].wind_load.get_zone('wall_corner').pressure.pos_uls,
...
def uls_wall_0_9D_1_4Wx(building: Building, snow_load: SnowLoad, zone_num: int):
    return {'uls 0.9D': building.wp * 0.9,
            'uls 1.4Wx (centre)': building.get_zone(zone_num)[0].wind_load.get_zone('wall_centre').pressure.neg_uls,
            'uls 1.4Wx (edge)': building.get_zone(zone_num)[0].wind_load.get_zone('wall_corner').pressure.neg_uls,
            'companion': snow_load.s_uls * 0.5}
...
def uls_wall_1_0D_1_0Ey(building: Building, snow_load: SnowLoad, zone_num: int):
    return {'uls 1.0D': building.wp,
            'uls 1.0Ey': building.get_zone(zone_num)[0].seismic_load.vp,
...
def uls_wall_1_0D_1_0Ex(building: Building, snow_load: SnowLoad, zone_num: int):
    return {'uls 1.0D': building.wp,
            'uls 1.0Ex': building.get_zone(zone_num)[0].seismic_load.vp,
            'companion': snow_load.s_uls * 0.25}
...
def sls_wall_1_0D_1_0Wy(building: Building, zone_num: int):
    return {'sls 1.0D': building.wp,
            'sls 1.0Wy (centre)': building.get_zone(zone_num)[0].wind_load.get_zone('wall_centre').pressure.pos_sls,
            'sls 1.0Wy (edge)': building.get_zone(zone_num)[0].wind_load.get_zone('wall_corner').pressure.pos_sls}
...
def sls_wall_1_0D_1_0Wx(building: Building, zone_num: int):
    return {'sls 1.0D': building.wp,
            'sls 1.0Wy (centre)': building.get_zone(zone_num)[0].wind_load.get_zone('wall_centre').pressure.neg_sls,
            'sls 1.0Wy (edge)': building.get_zone(zone_num)[0].wind_load.get_zone('wall_corner').pressure.neg_sls}
...
def uls_roof_1_25D_1_5L(building: Building, snow_load: SnowLoad):
    ...
    return {'uls 1.25D': building.roof.wp * 1.25,
            'uls 1.5L': 1.5,
            'companion (centre)': max(top_height_zone.wind_load.get_zone('roof_interior').pressure.pos_uls * 0.4, snow_load.s_uls),
            'companion (edge)': max(top_height_zone.wind_load.get_zone('roof_corner').pressure.pos_uls * 0.4, snow_load.s_uls)}
...
def uls_roof_0_9D_1_5L(building: Building, snow_load: SnowLoad):
    ...
    return {'uls 0.9D': building.roof.wp * 0.9,
            'uls 1.5L': 1.5,
            'companion (centre)': max(top_height_zone.wind_load.get_zone('roof_interior').pressure.neg_uls * 0.4, snow_load.s_uls),
            'companion (edge)': max(top_height_zone.wind_load.get_zone('roof_corner').pressure.neg_uls * 0.4, snow_load.s_uls)}
...
def sls_roof_1_0D_1_0Wy(building: Building, snow_load: SnowLoad):
    ...
    return {'sls 1.0D': building.roof.wp,
            ...
            'companion (centre)': max(top_height_zone.wind_load.get_zone('roof_interior').pressure.neg_sls * 0.3, snow_load.s_sls * 0.35),
            'companion (edge)': max(top_height_zone.wind_load.get_zone('roof_corner').pressure.neg_sls * 0.3, snow_load.s_sls * 0.35)}
...
def sls_roof_1_0D_1_0Wx(building: Building, snow_load: SnowLoad):
    ...
    return {'sls 1.0D': building.roof.wp,
            ...
            'companion (centre)': max(top_height_zone.wind_load.get_zone('roof_interior').pressure.pos_sls * 0.3, snow_load.s_sls * 0.35),
            'companion (edge)': max(top_height_zone.wind_load.get_zone('roof_corner').pressure.pos_sls * 0.3, snow_load.s_sls * 0.35)}
...
def compute_wall_load_combinations(building: Building, snow_load: SnowLoad, uls_wall_load_combination_type: ULSWallLoadCombinationTypes, sls_wall_load_combination_type: SLSWallLoadCombinationTypes):
        ...
        case (ULSWallLoadCombinationTypes.ULS_1_4_D, SLSWallLoadCombinationTypes.SLS_1_0D_1_0WY):
            ...
            for height_zone in sorted(building.zones.keys(), key=lambda x: x.zone_num, reverse=True):
                generate_wall_load_entries(columns=columns,
                                           ...
                                           uls_wall=uls_wall_1_4D(building),
...
def compute_roof_load_combinations(building: Building, snow_load: SnowLoad, uls_roof_load_combination_type: ULSRoofLoadCombinationTypes, sls_roof_load_combination_type: SLSRoofLoadCombinationTypes):
