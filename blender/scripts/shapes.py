...
def create_wind_cube(length=2.0, width=2.0, height=2.0, position=0, r=1.0, g=1.0):
    ...
    bpy.ops.mesh.primitive_cube_add(
        scale=(cube_x, cube_y, cube_z),
        size=1,
        enter_editmode=False,
        location=(0, 0, block_z),
    )
    ...
    # positive x
    bpy.ops.mesh.primitive_cube_add(
        scale=(length * 0.1 / 2, cube_y / 2, cube_z / 2),
        enter_editmode=True,
        location=(cube_x / 2 + length * 0.05, 0, block_z),
    )
    ...
    bpy.ops.mesh.primitive_cube_add(
        scale=(cube_x / 2, width * 0.1 / 2, cube_z / 2),
        enter_editmode=True,
        location=(0, cube_y / 2 + width * 0.05, block_z),
    )
    bpy.ops.mesh.primitive_cube_add(
        scale=(cube_x / 2, width * 0.1 / 2, cube_z / 2),
        enter_editmode=True,
    ...
    bpy.ops.mesh.primitive_cube_add(
        size=2,
        enter_editmode=True,
        location=(cube_x / 2 + length * 0.05, cube_y / 2 + width * 0.05, block_z),
        scale=(0.1, 0.1, cube_z / 2),
    )
    bpy.ops.mesh.primitive_cube_add(
        size=2,
        enter_editmode=True,
        ...
        if obj.type == "MESH":
            ...
            bpy.ops.mesh.extrude_context_move(
                ...
                TRANSFORM_OT_translate={
                    "value": (0, 0, -(total_height - triangle_height))
                },
            )
...
def create_axis(radius=0.05, depth=1, location=(0, -3, 2)):
    ...
    obj = add_axis_text(
        "X", location=(location[0] + depth, location[1] - 0.3, location[2])
    )
    obj.name = "xlabel"
