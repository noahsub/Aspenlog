...
def main():
    ...
    for i in range(len(data)):
        ...
        add_load_text(
            centre_pos, (0 - text_position_factor, -1.2, position), scale=scale
        )
        add_load_text(
            corner_pos, (1 - text_position_factor, -1.2, position), scale=scale
        )
        add_load_text(
            corner_pos, (-1 - text_position_factor, -1.2, position), scale=scale
        )
        ...
        add_load_text(
            centre_neg,
            (1.15, 0 - text_position_factor, position - 0.5),
            negative=True,
            scale=scale,
        )
        add_load_text(
            corner_neg,
            (1.15, -1 - text_position_factor, position - 0.5),
            negative=True,
            scale=scale,
        )
        add_load_text(
            corner_neg,
            (1.15, 0.9 - text_position_factor, position - 0.5),
            negative=True,
            scale=scale,
        )
        max_height += height
